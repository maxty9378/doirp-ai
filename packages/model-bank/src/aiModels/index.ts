...
const buildDefaultModelList = (map: ModelsMap): LobeDefaultAiModelListItem[] => {
  ...
  Object.entries(map).forEach(([provider, providerModels]) => {
    ...
    if (provider === 'google') {
      list = providerModels.filter((m) => m.id === GOOGLE_SINGLE_MODEL_ID);
    }
    const newModels = list.map((model) => ({
      ...model,
      abilities: model.abilities ?? {},
      enabled: model.enabled || (provider === 'google' && model.id === GOOGLE_SINGLE_MODEL_ID),
      providerId: provider,
      source: 'builtin',
    }));
