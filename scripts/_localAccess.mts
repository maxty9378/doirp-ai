...
interface LocalServerAccessFile {
  proxies?: {
    voiceCallFallbackEntries?: string[];
  };
  servers?: {
    voiceGateway?: Partial<SshAccessConfig>;
    vpnNode?: Partial<SshAccessConfig>;
  };
}

const LOCAL_SERVER_ACCESS_PATH = path.join(process.cwd(), '.local', 'server-access.json');

let cachedLocalAccessConfig: LocalServerAccessFile | null = null;
...
async function loadLocalAccessConfig() {
  ...
  try {
    ...
    cachedLocalAccessConfig = JSON.parse(raw) as LocalServerAccessFile;
    ...
    throw new Error(
      `[server-access] Failed to read ${LOCAL_SERVER_ACCESS_PATH}: ${nodeError.message}`,
    );
  }
}
...
async function resolveSshConfig(
  serverKey: keyof NonNullable<LocalServerAccessFile['servers']>,
  envPrefix: string,
): Promise<SshAccessConfig> {
...
export function proxyEntryToSocksUrl(entry: string) {
