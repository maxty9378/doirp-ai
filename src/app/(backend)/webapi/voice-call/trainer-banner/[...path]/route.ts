...
const resolveCandidateKeys = (segments: string[]) => {
  ...
  const prefixed = `voice-call/trainer-banner/${joined}`;
  const withPrefixRemoved = joined.replace(/^voice-call\/trainer-banner\//, '');

  return unique([prefixed, joined, `voice-call/trainer-banner/${withPrefixRemoved}`]).filter(
    Boolean,
  );
};
