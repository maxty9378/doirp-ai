...
const Agent = memo<AgentProps>(({ itemKey }) => {
  ...
  // Create menu items (only for admin)
  ...
  const emptyMenu = useMemo(() => [] as typeof dropdownMenu, []);
  const menuItems = isAdmin ? dropdownMenu : emptyMenu;

  return (
    <AccordionItem
      action={isAdmin ? <Actions dropdownMenu={dropdownMenu} isLoading={isLoading} /> : undefined}
      itemKey={itemKey}
      paddingBlock={4}
      paddingInline={'8px 4px'}
      headerWrapper={(header) => (
        <ContextMenuTrigger items={menuItems}>{header}</ContextMenuTrigger>
      )}
      title={
