...
import { compressImageForBanner } from '../../home/features/TrainingAgents/compressBannerImage';
...
const useStyles = createStyles(({ css }) => ({
  ...
  saveBar: css`
    @media (width <= 640px) {
      ...
      padding-block: 12px calc(env(safe-area-inset-bottom, 0px) + 8px);
      padding-inline: 0;

      background: linear-gradient(180deg, rgb(0 0 0 / 0%), ${cssVar.colorBgLayout} 28%);
    }
  `,
  knowledgeCards: css`
    display: flex;
...
const toTrainingBannerUrl = (path: string) => {
  ...
  const bannerPrefix = 'voice-call/trainer-banner/';
  const avatarPrefix = 'voice-call/trainer-avatar/';

  if (normalizedPath.startsWith(bannerPrefix)) {
    const keyTail = normalizedPath.slice(bannerPrefix.length);
    return `/webapi/voice-call/trainer-banner/${keyTail}`;
  }

  if (normalizedPath.startsWith(avatarPrefix)) {
    const keyTail = normalizedPath.slice(avatarPrefix.length);
    return `/webapi/voice-call/trainer-avatar/${keyTail}`;
  }
...
const groupIcon = (IconComp: typeof Settings, color?: string) => (
  <Icon icon={IconComp} size={18} style={{ color: color || cssVar.colorPrimary }} />
);
...
const TrainingScenarioEditor = memo<TrainingScenarioEditorProps>(
  ({ initialKey, hideSelector, mobile = false }) => {
    ...
    const colorTextSecondary = cssVar.colorTextSecondary;
    const colorPrimary = cssVar.colorPrimary;
        ...
        try {
          ...
          const text =
            error instanceof Error
              ? error.message
              : 'Не удалось загрузить баннер, попробуйте позже';
          message.error(text);
        } finally {
    ...
    const handleAvatarFileSelected = useCallback(
      async (file?: File | null) => {
        ...
        try {
          const compressed = await compressImageForBanner(file); // reuse compression
          const { data } = await uploadService.uploadFileToS3(compressed, {
            directory: 'voice-call/trainer-avatar',
          });
          const uploadedUrl = toTrainingBannerUrl(data.path);
          form.setFieldsValue({ avatarUrl: uploadedUrl });
          message.success('Аватарка загружена, не забудьте сохранить сценарий');
        } catch (error) {
          const text =
            error instanceof Error
              ? error.message
              : 'Не удалось загрузить аватарку, попробуйте позже';
          message.error(text);
        } finally {
      ...
      async (key: string) => {
        ...
        try {
          ...
          form.setFieldsValue({
            ...
            avatarUrl: data.scenario.avatarUrl ?? '',
            contextWindow: data.scenario.contextWindow ?? 5,
            sessionDurationSec: Math.round(
              (data.scenario.sessionDurationMs ?? data.scenario.silenceHardHangupMs ?? 300_000) /
                1000,
            ),
            ...
            silenceHardHangupMs: Math.round((data.scenario.silenceHardHangupMs ?? 300_000) / 1000),
    ...
    const openKnowledgeModal = useCallback(
      (mode: 'add' | 'edit', entry?: KnowledgeEntry) => {
    ...
    const basicGroup: FormGroupItemType = useMemo(
      () => ({
        ...
        children: [
          ...
          {
            label: 'Баннер',
            desc: 'Загрузите картинку баннера (сжимается автоматически)',
            children: (
          ...
          {
            label: 'Аватарка',
            desc: 'Маленькая аватарка персонажа (сжимается автоматически)',
            children: (
              <Flexbox horizontal align="center" gap={8}>
                <Input placeholder="https://..." style={{ flex: 1 }} />
                <Button
        ...
        children: [
          {
            ...
            children: (
              <Input.TextArea autoSize={{ minRows: 3, maxRows: 8 }} placeholder="Текст легенды" />
            ),
            name: 'legend',
          },
          {
        ...
        children: [
          {
            ...
            children: (
              <Input.TextArea
                ...
                placeholder="Инструкции для ИИ-агента"
              />
            ),
            name: 'systemPrompt',
          },
          ...
          },
          {
            ...
            children: (
              <InputNumber max={20} min={1} style={{ width: isMobileLayout ? '100%' : 120 }} />
            ),
            name: 'contextWindow',
            minWidth: undefined,
          },
      ...
      () => ({
        ...
        children: [
          ...
          },
          {
            ...
            children: (
              <Input.TextArea autoSize={{ minRows: 3, maxRows: 6 }} placeholder="Цель 1\nЦель 2" />
            ),
            name: 'goals',
          },
          {
          ...
          },
          {
            ...
            desc: 'Текст над шкалой оценки (напр. «Результат»)',
            children: <Input placeholder="Результат" />,
        ...
        children: [
          ...
          },
          {
            ...
            children: (
              <Input.TextArea
                autoSize={{ minRows: 2, maxRows: 4 }}
                placeholder="Введите позывной или реальное имя..."
              />
            ),
          ...
          },
          {
            ...
            children: (
              <Input.TextArea
                ...
                placeholder="Можно указать реальное имя..."
              />
            ),
        ...
        children: [
          ...
          },
          {
            ...
            children: (
              <Input.TextArea
                ...
                placeholder="Через 15 секунд наш эфир подходит к концу..."
              />
            ),
          ...
          },
          {
            ...
            children: (
              <Input.TextArea
                autoSize={{ minRows: 2, maxRows: 4 }}
                placeholder='Собеседник молчит. Скажи: "{{phrase}}".'
              />
            ),
          ...
          },
          {
            ...
            children: (
              <Input.TextArea
                ...
                placeholder="Отвечай короче: 1-2 предложения..."
              />
            ),
          ...
          },
          {
            ...
            children: (
              <Input.TextArea
                autoSize={{ minRows: 1, maxRows: 3 }}
                placeholder="Собеседник говорит очень тихо..."
              />
            ),
          ...
          },
          {
            ...
            children: (
              <Input.TextArea
                autoSize={{ minRows: 2, maxRows: 4 }}
                placeholder="Маркетолог справился. Признай поражение..."
              />
            ),
    ...
    return (
      <div className={styles.wrap}>
        ...
        <div
          ...
          style={{
            marginBottom: 12,
            maxWidth: isMobileLayout ? '100%' : FORM_STYLE.style?.maxWidth,
          }}
        >
        ...
        {loading && (
          <div style={{ color: cssVar.colorTextSecondary, marginTop: 12 }}>
              ...
              {isMobileLayout ? (
                <div className={styles.knowledgeCards}>
                  {payload.knowledgeEntries.map((record) => (
                    <div className={styles.knowledgeCard} key={record.id}>
        ...
        >
          <Form form={knowledgeForm} layout="vertical" style={{ marginTop: 16 }}>
            ...
            >
              <Input.TextArea
                autoSize={{ minRows: 2, maxRows: 4 }}
                placeholder="Бодрящий эффект, контролируемая рецептура..."
              />
            </Form.Item>
            <Form.Item
              label="Миф для атаки"
            ...
            >
              <Input.TextArea
                autoSize={{ minRows: 2, maxRows: 4 }}
                placeholder="Ударные дозы кофеина перегружают сердце..."
              />
            </Form.Item>
          </Form>
        </AntModal>
