...
/** Default wrapper: use div so dev tools (e.g. data-insp-path) never hit React.Fragment */
const DefaultWrapper = ({ children }: { children: React.ReactNode }) => <div style={{ display: 'contents' }}>{children}</div>;
...
export const PortalContent = memo<PortalContentProps>(({ renderBody }) => {
  ...
  const Wrapper = ViewImpl?.Wrapper ?? DefaultWrapper;
