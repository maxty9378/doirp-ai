    ...
    return {
      ...
      onComplete: async ({ finalState, reason }) => {
        ...
        try {
          ...
          const totalTokens = this.calculateTotalTokens(finalState.usage);
          if (totalTokens && this.userId) {
            try {
              await trackTokenUsage(this.userId, totalTokens);
              log('execSubAgentTask: tracked %d tokens for user %s', totalTokens, this.userId);
            } catch (error) {
              log('execSubAgentTask: failed to track token usage: %O', error);
            }
          }
        } catch (error) {
